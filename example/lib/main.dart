  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        theme: ThemeData(
          ...
          colorScheme: ColorScheme.fromSeed(
              seedColor: studioGreen, primary: studioGreen),
...
class _ExampleGalleryState extends State<ExampleGallery> {
  ...
  String get _endLabel => _custom || _rtl
      ? widget.examples[_selected].endLabel
      : '${_end.family} · ${_end.label}';
  ...
  Widget build(BuildContext context) => Scaffold(
        body: SafeArea(
          ...
          padding:
              EdgeInsets.all(MediaQuery.sizeOf(context).width < 600 ? 16 : 36),
          child: Center(
            ...
            child: Column(
                ...
                children: [
                  ...
                  Wrap(spacing: 10, runSpacing: 10, children: [
                    for (var index = 0; index < widget.examples.length; index++)
                      ChoiceChip(
                          ...
                          label: Text(
                              '0${index + 1}  ${widget.examples[index].title}'),
                      ...
                      final controls = StudioPanel(
                          child: Column(
                            ...
                            if (!_custom) ...[
                              ...
                              Text(
                                  _rtl
                                      ? '一个 Icon，分别生成 LTR / RTL 路径。'
                                      : '起点和终点都可以独立选择图标。',
                                  style: const TextStyle(
                                      fontSize: 12, color: Color(0xFF697A73))),
                              ...
                              if (_rtl)
                                ...
                                _picker('start-icon', 'START / 起始图标', _start,
                                    iconChoices, (value) => _start = value),
                                const SizedBox(height: 16),
                                _picker('end-icon', 'END / 目标图标', _end,
                                    iconChoices, (value) => _end = value),
                              ],
                              const Padding(
                  ...
                  const SizedBox(height: 28),
                  const Text('PATH COMBINER  /  轮廓之间，探索更多可能',
                      textAlign: TextAlign.center,
                      style: TextStyle(
                          fontSize: 10,
                          letterSpacing: 1.5,
                          color: Color(0xFF697A73))),
                ]),
          )),
        )),
      );

  Widget _picker(String key, String label, IconChoice value,
          List<IconChoice> choices, ValueChanged<IconChoice> update) =>
          ...
          for (final choice in choices)
            DropdownMenuItem(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Text('${choice.family} · ${choice.label}',
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData) {
            return const SizedBox(
                height: 360,
                child: Center(
                    child: CircularProgressIndicator(color: studioMint)));
          }
          return Column(children: [
            const Align(
            ...
            const SizedBox(height: 20),
            Text(_showEnd ? _endLabel : _startLabel,
            ...
            Text(
                _rtl
                    ? '仅 matchTextDirection 为 true 的图标会镜像。'
                    : '预览为字体轮廓描边，不是实心 Icon。',
                textAlign: TextAlign.center,
                style: const TextStyle(fontSize: 11, color: Color(0xFF93B6A7))),
          ]);
