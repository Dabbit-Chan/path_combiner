...
void main() {
  ...
  test('retains quadratic curves and implied on-curve points', () async {
    final curve = await const IconData(0xe002).toPath(fontData: font, size: 100);
  ...
  test('reads scaled compound glyphs and point attachments', () async {
    final scaled = await const IconData(0xe003).toPath(fontData: font, size: 150);
    expect(scaled.getBounds(), const Rect.fromLTWH(0, 25, 150, 100));
    expect(scaled.computeMetrics().length, 2);
    final attached = await const IconData(0xe006).toPath(fontData: font, size: 200);
    expect(attached.getBounds(), const Rect.fromLTWH(0, 50, 200, 100));
    expect(attached.computeMetrics().length, 2);
  });

  test('supports short loca, cmap glyph arrays and repeated short flags', () async {
    ...
    expect(
      compact.computeMetrics().single.length,
  ...
  test('blank glyph returns an empty path; missing glyph throws', () async {
    ...
    await expectLater(
      const IconData(0x1234).toPath(fontData: font),
      throwsStateError,
    );
  });
  ...
  test('rejects invalid geometry and conflicting font sources', () async {
    for (final size in [0.0, -1.0, double.nan, double.infinity]) {
      await expectLater(
        triangle.toPath(fontData: font, size: size),
        throwsArgumentError,
      );
    }
    await expectLater(
      triangle.toPath(offset: const Offset(double.nan, 0)),
      throwsArgumentError,
    );
    await expectLater(
      triangle.toPath(fontAsset: 'font.ttf', fontData: font),
      throwsArgumentError,
    );
  ...
  test('rejects malformed, unsupported and cyclic fonts', () async {
    await expectLater(
      triangle.toPath(fontData: ByteData(3)),
    ...
    await expectLater(
      const IconData(0xe007).toPath(fontData: font),
      throwsFormatException,
    );
    final truncated = ByteData.sublistView(font.buffer.asUint8List(), 0, 100);
    await expectLater(
      triangle.toPath(fontData: truncated),
      throwsFormatException,
    );
  });
  ...
  test('resolves Material font, caches data and returns independent paths', () async {
  ...
  test('resolves package fonts and explicit assets', () async {
    ...
    final converter = IconPathConverter(bundle: bundle);
    const icon = IconData(0xe000, fontFamily: 'TestIcons', fontPackage: 'my_icons');
    expect((await converter.convert(icon)).computeMetrics(), isNotEmpty);
    await converter.convert(icon);
  ...
  test('asset load failures can be retried', () async {
    ...
    final converter = IconPathConverter(bundle: bundle);
    await expectLater(
      converter.convert(triangle, fontAsset: 'font.ttf'),
      throwsStateError,
    );
    bundle.assets['font.ttf'] = font;
    expect(
      (await converter.convert(triangle, fontAsset: 'font.ttf')).computeMetrics(),
      isNotEmpty,
    );
  });
  ...
  test('converts real Material Icons from the Flutter SDK', () async {
    ...
    final codePoints = await File.fromUri(fontFile.uri.resolve('codepoints')).readAsLines();
    final uniqueCodePoints = codePoints
        .where((line) => line.trim().isNotEmpty)
        .map(
          (line) => int.parse(line.trim().split(RegExp(r'\s+')).last, radix: 16),
        )
        .toSet();
  ...
  testWidgets('converted outlines animate in PathCombiner', (tester) async {
    ...
    final second = await const IconData(0xe001).toPath(fontData: font, size: 100);
